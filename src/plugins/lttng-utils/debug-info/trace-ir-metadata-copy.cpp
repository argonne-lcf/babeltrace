...
copy_trace_class_content(struct trace_ir_maps *ir_maps, const bt_trace_class *in_trace_class,
                         ...
                         bt_self_component *self_comp)
{
    ...
    BT_COMP_LOGD("Copying content of trace class: in-tc-addr=%p, out-tc-addr=%p", in_trace_class,
                 out_trace_class);
    ...
    bt_trace_class_set_assigns_automatic_stream_class_id(out_trace_class, BT_FALSE);
    ...
    for (sc_idx = 0; sc_idx < sc_number; sc_idx++) {
        ...
        const bt_stream_class *in_stream_class =
            bt_trace_class_borrow_stream_class_by_index_const(in_trace_class, sc_idx);

        out_stream_class = trace_ir_mapping_borrow_mapped_stream_class(ir_maps, in_stream_class);
        if (!out_stream_class) {
            out_stream_class =
                trace_ir_mapping_create_new_mapped_stream_class(ir_maps, in_stream_class);
            if (!out_stream_class) {
    ...
    BT_COMP_LOGD("Copied content of trace class: in-tc-addr=%p, out-tc-addr=%p", in_trace_class,
                 out_trace_class);
...
static bt_clock_class *borrow_mapped_clock_class(struct trace_ir_metadata_maps *md_maps,
                                                 const bt_clock_class *in_clock_class)
{
    BT_ASSERT_DBG(md_maps);
    BT_ASSERT_DBG(in_clock_class);

    return static_cast<bt_clock_class *>(
        g_hash_table_lookup(md_maps->clock_class_map, (gpointer) in_clock_class));
}

static bt_clock_class *create_new_mapped_clock_class(bt_self_component *self_comp,
                                                     ...
                                                     const bt_clock_class *in_clock_class)
{
    ...
    BT_COMP_LOGD("Creating new mapped clock class: in-cc-addr=%p", in_clock_class);
    ...
    if (!out_clock_class) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Cannot create clock class");
        goto end;
    }
    /* If not, create a new one and add it to the mapping. */
    status = copy_clock_class_content(in_clock_class, out_clock_class, log_level, self_comp);
    if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Cannot copy clock class");
        BT_CLOCK_CLASS_PUT_REF_AND_RESET(out_clock_class);
        goto end;
    }

    g_hash_table_insert(md_maps->clock_class_map, (gpointer) in_clock_class, out_clock_class);

    BT_COMP_LOGD("Created new mapped clock class: in-cc-addr=%p, out-cc-addr=%p", in_clock_class,
                 out_clock_class);
end:
...
enum debug_info_trace_ir_mapping_status
copy_stream_class_content(struct trace_ir_maps *ir_maps, const bt_stream_class *in_stream_class,
                          bt_stream_class *out_stream_class)
{
    enum debug_info_trace_ir_mapping_status status;
    struct trace_ir_metadata_maps *md_maps;
    ...
    in_clock_class = bt_stream_class_borrow_default_clock_class_const(in_stream_class);

    if (in_clock_class) {
        enum bt_stream_class_set_default_clock_class_status set_def_cc_status;
        /* Copy the clock class. */
        out_clock_class = borrow_mapped_clock_class(md_maps, in_clock_class);
        if (!out_clock_class) {
            out_clock_class =
                create_new_mapped_clock_class(ir_maps->self_comp, md_maps, in_clock_class);
            if (!out_clock_class) {
                status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
                goto end;
            }
        ...
        set_def_cc_status =
            bt_stream_class_set_default_clock_class(out_stream_class, out_clock_class);
    ...
    bt_stream_class_set_user_attributes(
        out_stream_class, bt_stream_class_borrow_user_attributes_const(in_stream_class));

    bt_stream_class_set_supports_packets(
        out_stream_class, bt_stream_class_supports_packets(in_stream_class),
        bt_stream_class_packets_have_beginning_default_clock_snapshot(in_stream_class),
        bt_stream_class_packets_have_end_default_clock_snapshot(in_stream_class));
    bt_stream_class_set_supports_discarded_events(
        out_stream_class, bt_stream_class_supports_discarded_events(in_stream_class),
        bt_stream_class_discarded_events_have_default_clock_snapshots(in_stream_class));
    ...
    if (graph_mip_version == 1) {
        ...
        /*
         * Do not copy the stream class UID as the trace may be
         * modified and should no longer have the same identifier.
         */
    }

    bt_stream_class_set_assigns_automatic_stream_id(out_stream_class, BT_FALSE);
    bt_stream_class_set_assigns_automatic_event_class_id(out_stream_class, BT_FALSE);

    /*
     * Add the input packet context field class to the context to
     * resolution in the further steps.
     */
    in_packet_context_fc = bt_stream_class_borrow_packet_context_field_class_const(in_stream_class);
    md_maps->fc_resolving_ctx->packet_context = in_packet_context_fc;

    if (in_packet_context_fc) {
        enum bt_stream_class_set_field_class_status set_fc_status;
        /* Copy packet context. */
        out_packet_context_fc = create_field_class_copy(md_maps, in_packet_context_fc);

        status = copy_field_class_content(md_maps, in_packet_context_fc, out_packet_context_fc);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
        ...
        set_fc_status =
            bt_stream_class_set_packet_context_field_class(out_stream_class, out_packet_context_fc);
    ...
    in_common_context_fc =
        bt_stream_class_borrow_event_common_context_field_class_const(in_stream_class);
    ...
    if (in_common_context_fc) {
         ...
         * abstract that.*/
        out_common_context_fc = create_field_class_copy(md_maps, in_common_context_fc);
        status = copy_event_common_context_field_class_content(
            md_maps, ir_maps->debug_info_field_class_name, in_common_context_fc,
        ...
        set_fc_status = bt_stream_class_set_event_common_context_field_class(out_stream_class,
                                                                             out_common_context_fc);
        if (set_fc_status != BT_STREAM_CLASS_SET_FIELD_CLASS_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
    ...
    for (ec_idx = 0; ec_idx < ec_number; ec_idx++) {
        ...
        const bt_event_class *in_event_class =
            bt_stream_class_borrow_event_class_by_id_const(in_stream_class, ec_idx);
        out_event_class = trace_ir_mapping_borrow_mapped_event_class(ir_maps, in_event_class);
        if (!out_event_class) {
            /*
             ...
             */
            out_event_class =
                trace_ir_mapping_create_new_mapped_event_class(ir_maps, in_event_class);
            if (!out_event_class) {
                status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
                goto end;
            }
    ...
    BT_COMP_LOGD("Copied content of stream class: in-sc-addr=%p, out-sc-addr=%p", in_stream_class,
                 out_stream_class);
...
enum debug_info_trace_ir_mapping_status
copy_event_class_content(struct trace_ir_maps *ir_maps, const bt_event_class *in_event_class,
                         bt_event_class *out_event_class)
{
    enum debug_info_trace_ir_mapping_status status;
    struct trace_ir_metadata_maps *md_maps;
    ...
    prop_avail = bt_event_class_get_log_level(in_event_class, &ec_log_level);
    ...
    in_event_specific_context =
        bt_event_class_borrow_specific_context_field_class_const(in_event_class);

    md_maps->fc_resolving_ctx->event_specific_context = in_event_specific_context;
    ...
    if (in_event_specific_context) {
        ...
        out_specific_context_fc = create_field_class_copy(md_maps, in_event_specific_context);

        status =
            copy_field_class_content(md_maps, in_event_specific_context, out_specific_context_fc);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
         ...
         * class.
         */
        set_fc_status = bt_event_class_set_specific_context_field_class(out_event_class,
     ...
     * the context.
     */
    in_event_payload = bt_event_class_borrow_payload_field_class_const(in_event_class);
    ...
    if (in_event_payload) {
        ...
        out_payload_fc = create_field_class_copy(md_maps, in_event_payload);
        status = copy_field_class_content(md_maps, in_event_payload, out_payload_fc);
        if (status != DEBUG_INFO_TRACE_IR_MAPPING_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
                                      "Error copying event class' specific context field class:"
        ...
        set_fc_status = bt_event_class_set_payload_field_class(out_event_class, out_payload_fc);
        if (set_fc_status != BT_EVENT_CLASS_SET_FIELD_CLASS_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp,
    ...
    BT_COMP_LOGD("Copied content of event class: in-ec-addr=%p, out-ec-addr=%p", in_event_class,
                 out_event_class);
...
enum debug_info_trace_ir_mapping_status copy_event_common_context_field_class_content(
    struct trace_ir_metadata_maps *md_maps, const char *debug_info_fc_name,
    const bt_field_class *in_field_class, bt_field_class *out_field_class)
{
    ...
    if (is_event_common_ctx_dbg_info_compatible(in_field_class, debug_info_fc_name)) {
         ...
         * classes.
         */
        debug_field_class = bt_field_class_structure_create(md_maps->output_trace_class);
        if (!debug_field_class) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to create debug_info structure.");
            status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
            goto error;
        }

        bin_field_class = bt_field_class_string_create(md_maps->output_trace_class);
        if (!bin_field_class) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to create string for field=\"bin\".");
            status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
            goto error;
        }

        func_field_class = bt_field_class_string_create(md_maps->output_trace_class);
        if (!func_field_class) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to create string for field=\"func\".");
            status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
            goto error;
        }

        src_field_class = bt_field_class_string_create(md_maps->output_trace_class);
        if (!src_field_class) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to create string for field=\"src\".");
            status = DEBUG_INFO_TRACE_IR_MAPPING_STATUS_MEMORY_ERROR;
            goto error;
        }

        append_member_status =
            bt_field_class_structure_append_member(debug_field_class, "bin", bin_field_class);
        if (append_member_status != BT_FIELD_CLASS_STRUCTURE_APPEND_MEMBER_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to add a field to debug_info struct: "
                                                 "field=\"bin\".");
        ...
        append_member_status =
            bt_field_class_structure_append_member(debug_field_class, "func", func_field_class);
        if (append_member_status != BT_FIELD_CLASS_STRUCTURE_APPEND_MEMBER_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to add a field to debug_info struct: "
                                                 "field=\"func\".");
            status = static_cast<debug_info_trace_ir_mapping_status>(append_member_status);
        ...
        append_member_status =
            bt_field_class_structure_append_member(debug_field_class, "src", src_field_class);
        if (append_member_status != BT_FIELD_CLASS_STRUCTURE_APPEND_MEMBER_STATUS_OK) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to add a field to debug_info struct: "
                                                 "field=\"src\".");
            status = static_cast<debug_info_trace_ir_mapping_status>(append_member_status);
...
enum debug_info_trace_ir_mapping_status
copy_field_class_content(struct trace_ir_metadata_maps *md_maps,
                         const bt_field_class *in_field_class, bt_field_class *out_field_class)
{
    return copy_field_class_content_internal(md_maps, in_field_class, out_field_class);
}
