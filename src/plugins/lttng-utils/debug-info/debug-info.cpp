...
static int debug_info_init(struct debug_info *info)
{
    info->q_statedump_bin_info = g_quark_from_string("lttng_ust_statedump:bin_info");
    info->q_statedump_debug_link = g_quark_from_string("lttng_ust_statedump:debug_link");
    info->q_statedump_build_id = g_quark_from_string("lttng_ust_statedump:build_id");
    info->q_statedump_start = g_quark_from_string("lttng_ust_statedump:start");
...
static void debug_info_source_destroy(struct debug_info_source *debug_info_src)
{
    if (!debug_info_src) {
        return;
    }
...
debug_info_source_create_from_bin(struct bin_info *bin, uint64_t ip, bt_self_component *self_comp)
{
    ...
    if (src_loc) {
        ...
        if (!debug_info_src->line_no) {
            BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Error occurred when setting `line_no` field.");
            goto error;
        }
        ...
        if (src_loc->filename) {
            ...
            debug_info_src->short_src_path = get_filename_from_path(debug_info_src->src_path);
        }
    ...
    if (bin->elf_path) {
        ...
        debug_info_src->short_bin_path = get_filename_from_path(debug_info_src->bin_path);
...
static struct proc_debug_info_sources *proc_debug_info_sources_create(void)
{
    ...
    proc_dbg_info_src->baddr_to_bin_info = g_hash_table_new_full(
        g_int64_hash, g_int64_equal, (GDestroyNotify) g_free, (GDestroyNotify) bin_info_destroy);
    if (!proc_dbg_info_src->baddr_to_bin_info) {
        goto error;
    }
...
static inline const bt_field *event_borrow_payload_field(const bt_event *event,
                                                         const char *field_name)
{
    ...
    field = bt_field_structure_borrow_member_field_by_name_const(event_payload, field_name);
    return field;
}
                                                                ...
                                                                const char *field_name)
{
    ...
    field = bt_field_structure_borrow_member_field_by_name_const(event_common_ctx, field_name);

end:
    return field;
}

static inline void event_get_common_context_signed_integer_field_value(const bt_event *event,
                                                                       const char *field_name,
                                                                       int64_t *value)
{
    *value =
...
static inline void event_get_payload_build_id_length(const bt_event *event, const char *field_name,
                                                     uint64_t *build_id_len)
{
    ...
    BT_ASSERT(bt_field_class_get_type(bt_field_class_array_borrow_element_field_class_const(
                  build_id_field_class)) == BT_FIELD_CLASS_TYPE_UNSIGNED_INTEGER);

    *build_id_len = bt_field_array_get_length(build_id_field);
}

static inline void event_get_payload_build_id_value(const bt_event *event, const char *field_name,
                                                    uint8_t *build_id)
{
    ...
    for (i = 0; i < build_id_len; i++) {
        curr_field = bt_field_array_borrow_element_field_by_index_const(build_id_field, i);

        build_id[i] = bt_field_integer_unsigned_get_value(curr_field);
    }
}

static void event_get_payload_unsigned_integer_field_value(const bt_event *event,
                                                           const char *field_name, uint64_t *value)
{
    *value = bt_field_integer_unsigned_get_value(event_borrow_payload_field(event, field_name));
}

static void event_get_payload_string_field_value(const bt_event *event, const char *field_name,
                                                 const char **value)
{
    *value = bt_field_string_get_value(event_borrow_payload_field(event, field_name));
}

static inline bool event_has_payload_field(const bt_event *event, const char *field_name)
{
                                  ...
                                  struct proc_debug_info_sources *proc_dbg_info_src, uint64_t ip)
{
    ...
    *((uint64_t *) key) = ip;
    ...
    debug_info_src = static_cast<debug_info_source *>(
        g_hash_table_lookup(proc_dbg_info_src->ip_to_debug_info_src, key));
    if (debug_info_src) {
        goto end;
    }
    ...
    while (g_hash_table_iter_next(&iter, &baddr, &value)) {
         ...
         */
        debug_info_src = debug_info_source_create_from_bin(bin, ip, debug_info->self_comp);
        if (debug_info_src) {
                                                  ...
                                                  uint64_t ip)
{
    ...
    proc_dbg_info_src =
        proc_debug_info_sources_ht_get_entry(debug_info->vpid_to_proc_dbg_info_src, vpid);
    if (!proc_dbg_info_src) {
        goto end;
    }

    dbg_info_src = proc_debug_info_sources_get_entry(debug_info, proc_dbg_info_src, ip);

end:
    return dbg_info_src;
}

static struct debug_info *debug_info_create(struct debug_info_component *comp,
                                            const bt_trace *trace, struct bt_fd_cache *fdc)
{
    ...
    debug_info->vpid_to_proc_dbg_info_src =
        g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify) g_free,
...
static void debug_info_destroy(struct debug_info *debug_info)
{
    ...
    remove_listener_status = bt_trace_remove_destruction_listener(
        debug_info->input_trace, debug_info->destruction_listener_id);
...
static void handle_event_statedump_build_id(struct debug_info *debug_info, const bt_event *event)
{
    struct proc_debug_info_sources *proc_dbg_info_src;
    ...
    event_get_common_context_signed_integer_field_value(event, VPID_FIELD_NAME, &vpid);
    event_get_payload_unsigned_integer_field_value(event, BADDR_FIELD_NAME, &baddr);
    ...
    bin = static_cast<bin_info *>(
        g_hash_table_lookup(proc_dbg_info_src->baddr_to_bin_info, (gpointer) &baddr));
    if (!bin) {
        /*
    ...
    event_get_payload_build_id_length(event, BUILD_ID_FIELD_NAME, &build_id_len);
...
static void handle_event_statedump_debug_link(struct debug_info *debug_info, const bt_event *event)
{
    ...
    event_get_payload_unsigned_integer_field_value(event, CRC32_FIELD_NAME, &crc_field_value);

    crc32 = (uint32_t) crc_field_value;

    event_get_payload_string_field_value(event, FILENAME_FIELD_NAME, &filename);
...
static void handle_bin_info_event(struct debug_info *debug_info, const bt_event *event,
                                  bool has_pic_field)
{
    struct proc_debug_info_sources *proc_dbg_info_src;
    struct bin_info *bin;
    ...
    if (has_pic_field) {
        ...
        event_get_payload_unsigned_integer_field_value(event, IS_PIC_FIELD_NAME,
                                                       &is_pic_field_value);
    ...
    *((uint64_t *) key) = baddr;
    ...
    bin = bin_info_create(debug_info->fd_cache, path, baddr, memsz, is_pic,
                          debug_info->comp->arg_debug_dir, debug_info->comp->arg_target_prefix,
...
static inline void handle_event_statedump_bin_info(struct debug_info *debug_info,
                                                   const bt_event *event)
{
    handle_bin_info_event(debug_info, event, true);
}

static inline void handle_event_lib_load(struct debug_info *debug_info, const bt_event *event)
{
    handle_bin_info_event(debug_info, event, false);
}

static void handle_event_lib_unload(struct debug_info *debug_info, const bt_event *event)
{
    gboolean ret;
    struct proc_debug_info_sources *proc_dbg_info_src;
    uint64_t baddr;
    ...
    ret = g_hash_table_remove(proc_dbg_info_src->baddr_to_bin_info, (gpointer) &baddr);
    BT_ASSERT(ret);
end:
    return;
}

static void handle_event_statedump_start(struct debug_info *debug_info, const bt_event *event)
{
    struct proc_debug_info_sources *proc_dbg_info_src;
    int64_t vpid;
...
static void trace_debug_info_remove_func(const bt_trace *in_trace, void *data)
{
    struct debug_info_msg_iter *debug_it = static_cast<debug_info_msg_iter *>(data);
    if (debug_it->debug_info_map) {
        gboolean ret;
        ret = g_hash_table_remove(debug_it->debug_info_map, (gpointer) in_trace);
        BT_ASSERT(ret);
    }
}

static void handle_event_statedump(struct debug_info_msg_iter *debug_it, const bt_event *event)
{
    ...
    trace = bt_stream_borrow_trace_const(bt_event_borrow_stream_const(event));

    debug_info =
        static_cast<struct debug_info *>(g_hash_table_lookup(debug_it->debug_info_map, trace));
    if (!debug_info) {
        bt_trace_add_listener_status add_listener_status;

        debug_info = debug_info_create(debug_it->debug_info_component, trace, &debug_it->fd_cache);
        g_hash_table_insert(debug_it->debug_info_map, (gpointer) trace, debug_info);
        add_listener_status = bt_trace_add_destruction_listener(
            trace, trace_debug_info_remove_func, debug_it, &debug_info->destruction_listener_id);
    ...
    } else if (q_event_name == debug_info->q_dl_open || q_event_name == debug_info->q_lib_load) {
        /*
...
static void destroy_debug_info_comp(struct debug_info_component *debug_info)
{
    if (!debug_info) {
        return;
    }
...
static void fill_debug_info_bin_field(struct debug_info_source *dbg_info_src, bool full_path,
                                      ...
                                      bt_self_component *self_comp)
{
    ...
    BT_ASSERT_DBG(bt_field_get_class_type(curr_field) == BT_FIELD_CLASS_TYPE_STRING);

    if (dbg_info_src) {
        if (full_path) {
            set_status = bt_field_string_set_value(curr_field, dbg_info_src->bin_path);
        } else {
            set_status = bt_field_string_set_value(curr_field, dbg_info_src->short_bin_path);
        }
        ...
        append_status = bt_field_string_append(curr_field, dbg_info_src->bin_loc);
...
static void fill_debug_info_func_field(struct debug_info_source *dbg_info_src, bt_field *curr_field,
                                       bt_logging_level log_level, bt_self_component *self_comp)
{
    ...
    if (dbg_info_src && dbg_info_src->func) {
        status = bt_field_string_set_value(curr_field, dbg_info_src->func);
    } else {
...
static void fill_debug_info_src_field(struct debug_info_source *dbg_info_src, bool full_path,
                                      bt_field *curr_field, bt_logging_level log_level,
                                      bt_self_component *self_comp)
{
    bt_field_string_set_value_status set_status;
    ...
    if (dbg_info_src && dbg_info_src->src_path) {
        if (full_path) {
            set_status = bt_field_string_set_value(curr_field, dbg_info_src->src_path);
        } else {
            set_status = bt_field_string_set_value(curr_field, dbg_info_src->short_src_path);
        }
        if (set_status != BT_FIELD_STRING_SET_VALUE_STATUS_OK) {
        ...
        append_status = bt_field_string_append(curr_field, dbg_info_src->line_no);
        if (append_status != BT_FIELD_STRING_APPEND_STATUS_OK) {
...
static void fill_debug_info_field_empty(bt_field *debug_info_field, bt_logging_level log_level,
                                        bt_self_component *self_comp)
{
    bt_field_string_set_value_status status;
    bt_field *bin_field, *func_field, *src_field;

    BT_ASSERT_DBG(bt_field_get_class_type(debug_info_field) == BT_FIELD_CLASS_TYPE_STRUCTURE);

    bin_field = bt_field_structure_borrow_member_field_by_name(debug_info_field, "bin");
    func_field = bt_field_structure_borrow_member_field_by_name(debug_info_field, "func");
    src_field = bt_field_structure_borrow_member_field_by_name(debug_info_field, "src");

    BT_ASSERT_DBG(bt_field_get_class_type(bin_field) == BT_FIELD_CLASS_TYPE_STRING);
    BT_ASSERT_DBG(bt_field_get_class_type(func_field) == BT_FIELD_CLASS_TYPE_STRING);
    BT_ASSERT_DBG(bt_field_get_class_type(src_field) == BT_FIELD_CLASS_TYPE_STRING);
...
static void fill_debug_info_field(struct debug_info *debug_info, int64_t vpid, uint64_t ip,
                                  bt_field *debug_info_field)
{
    ...
    BT_ASSERT_DBG(bt_field_class_structure_get_member_count(debug_info_fc) == 3);

    dbg_info_src = debug_info_query(debug_info, vpid, ip);

    fill_debug_info_bin_field(
        dbg_info_src, debug_info->comp->arg_full_path,
        bt_field_structure_borrow_member_field_by_name(debug_info_field, "bin"),
        debug_info->log_level, debug_info->self_comp);
    fill_debug_info_func_field(
        dbg_info_src, bt_field_structure_borrow_member_field_by_name(debug_info_field, "func"),
        debug_info->log_level, debug_info->self_comp);
    fill_debug_info_src_field(
        dbg_info_src, debug_info->comp->arg_full_path,
        bt_field_structure_borrow_member_field_by_name(debug_info_field, "src"),
        debug_info->log_level, debug_info->self_comp);
}

static void fill_debug_info_event_if_needed(struct debug_info_msg_iter *debug_it,
                                            const bt_event *in_event, bt_event *out_event)
{
    ...
    uint64_t vpid;
    int64_t ip;
    gchar *debug_info_field_name = debug_it->debug_info_component->arg_debug_info_field_name;
    bt_logging_level log_level = debug_it->log_level;
    bt_self_component *self_comp = debug_it->self_comp;

    in_common_ctx_field = bt_event_borrow_common_context_field_const(in_event);
    ...
    out_debug_info_field =
        bt_field_structure_borrow_member_field_by_name(out_common_ctx_field, debug_info_field_name);

    vpid_field =
        bt_field_structure_borrow_member_field_by_name_const(out_common_ctx_field, VPID_FIELD_NAME);
    ip_field =
        bt_field_structure_borrow_member_field_by_name_const(out_common_ctx_field, IP_FIELD_NAME);
     ...
     * resolving.
     */
    debug_info = static_cast<struct debug_info *>(
        g_hash_table_lookup(debug_it->debug_info_map,
                            bt_stream_borrow_trace_const(bt_event_borrow_stream_const(in_event))));
    ...
    if (debug_info) {
        ...
        fill_debug_info_field_empty(out_debug_info_field, log_level, self_comp);
    }
end:
    return;
}

static void update_event_statedump_if_needed(struct debug_info_msg_iter *debug_it,
...
static bt_message *handle_event_message(struct debug_info_msg_iter *debug_it,
                                        const bt_message *in_message)
{
    ...
    out_event_class = trace_ir_mapping_borrow_mapped_event_class(debug_it->ir_maps, in_event_class);
    if (!out_event_class) {
        out_event_class =
            trace_ir_mapping_create_new_mapped_event_class(debug_it->ir_maps, in_event_class);
    }
    ...
    if (default_cc) {
        ...
        cs = bt_message_event_borrow_default_clock_snapshot_const(in_message);

        /* Create an output event message. */
        if (out_packet) {
            out_message = bt_message_event_create_with_packet_and_default_clock_snapshot(
                debug_it->input_iterator, out_event_class, out_packet,
                bt_clock_snapshot_get_value(cs));
        } else {
            out_message = bt_message_event_create_with_default_clock_snapshot(
                debug_it->input_iterator, out_event_class, out_stream,
                bt_clock_snapshot_get_value(cs));
        }
    } else {
        if (out_packet) {
            out_message = bt_message_event_create_with_packet(debug_it->input_iterator,
                                                              out_event_class, out_packet);
        } else {
            out_message =
                bt_message_event_create(debug_it->input_iterator, out_event_class, out_stream);
        }
    }

    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Error creating output event message.");
        goto error;
    }
...
static bt_message *handle_stream_begin_message(struct debug_info_msg_iter *debug_it,
                                               const bt_message *in_message)
{
    const bt_stream *in_stream;
    ...
    out_message = bt_message_stream_beginning_create(debug_it->input_iterator, out_stream);
    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp,
...
static bt_message *handle_stream_end_message(struct debug_info_msg_iter *debug_it,
                                             const bt_message *in_message)
{
    const bt_stream *in_stream;
    bt_message *out_message = nullptr;
    ...
    out_message = bt_message_stream_end_create(debug_it->input_iterator, out_stream);
    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp,
...
static bt_message *handle_packet_begin_message(struct debug_info_msg_iter *debug_it,
                                               const bt_message *in_message)
{
    ...
    const bt_packet *in_packet = bt_message_packet_beginning_borrow_packet_const(in_message);
    BT_ASSERT(in_packet);

    /* This packet should not be already mapped. */
    BT_ASSERT(!trace_ir_mapping_borrow_mapped_packet(debug_it->ir_maps, in_packet));

    out_packet = trace_ir_mapping_create_new_mapped_packet(debug_it->ir_maps, in_packet);

    BT_ASSERT(out_packet);

    has_default_clock_snapshot = bt_stream_class_packets_have_beginning_default_clock_snapshot(
        bt_stream_borrow_class_const(bt_packet_borrow_stream_const(in_packet)));
    if (has_default_clock_snapshot) {
        /* Borrow clock snapshot. */
        cs = bt_message_packet_beginning_borrow_default_clock_snapshot_const(in_message);

        /* Create an output packet beginning message. */
        out_message = bt_message_packet_beginning_create_with_default_clock_snapshot(
            debug_it->input_iterator, out_packet, bt_clock_snapshot_get_value(cs));
    } else {
        out_message = bt_message_packet_beginning_create(debug_it->input_iterator, out_packet);
    }
    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp,
...
static bt_message *handle_packet_end_message(struct debug_info_msg_iter *debug_it,
                                             const bt_message *in_message)
{
    bool has_default_clock_snapshot;
    const bt_clock_snapshot *cs;
    ...
    has_default_clock_snapshot = bt_stream_class_packets_have_end_default_clock_snapshot(
        bt_stream_borrow_class_const(bt_packet_borrow_stream_const(in_packet)));
    if (has_default_clock_snapshot) {
        /* Borrow clock snapshot. */
        cs = bt_message_packet_end_borrow_default_clock_snapshot_const(in_message);

        /* Create an outpute packet end message. */
        out_message = bt_message_packet_end_create_with_default_clock_snapshot(
            debug_it->input_iterator, out_packet, bt_clock_snapshot_get_value(cs));
    } else {
        out_message = bt_message_packet_end_create(debug_it->input_iterator, out_packet);
    }
...
static bt_message *handle_msg_iterator_inactivity(const bt_message *in_message)
{
    /*
...
static bt_message *handle_discarded_events_message(struct debug_info_msg_iter *debug_it,
                                                   const bt_message *in_message)
{
    ...
    in_stream = bt_message_discarded_events_borrow_stream_const(in_message);
    BT_ASSERT(in_stream);

    out_stream = trace_ir_mapping_borrow_mapped_stream(debug_it->ir_maps, in_stream);
    BT_ASSERT(out_stream);

    has_default_clock_snapshots = bt_stream_class_discarded_events_have_default_clock_snapshots(
        bt_stream_borrow_class_const(in_stream));
    if (has_default_clock_snapshots) {
        begin_cs =
            bt_message_discarded_events_borrow_beginning_default_clock_snapshot_const(in_message);
        end_cs = bt_message_discarded_events_borrow_end_default_clock_snapshot_const(in_message);
        ...
        out_message = bt_message_discarded_events_create_with_default_clock_snapshots(
            debug_it->input_iterator, out_stream, begin_cs_value, end_cs_value);
    } else {
        out_message = bt_message_discarded_events_create(debug_it->input_iterator, out_stream);
    }
    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp,
    ...
    prop_avail = bt_message_discarded_events_get_count(in_message, &discarded_events);

    if (prop_avail == BT_PROPERTY_AVAILABILITY_NOT_AVAILABLE) {
        bt_message_discarded_events_set_count(out_message, discarded_events);
    }
...
static bt_message *handle_discarded_packets_message(struct debug_info_msg_iter *debug_it,
                                                    const bt_message *in_message)
{
    const bt_clock_snapshot *begin_cs, *end_cs;
    bool has_default_clock_snapshots;
    ...
    in_stream = bt_message_discarded_packets_borrow_stream_const(in_message);
    BT_ASSERT(in_stream);

    out_stream = trace_ir_mapping_borrow_mapped_stream(debug_it->ir_maps, in_stream);
    BT_ASSERT(out_stream);

    has_default_clock_snapshots = bt_stream_class_discarded_packets_have_default_clock_snapshots(
        bt_stream_borrow_class_const(in_stream));
    if (has_default_clock_snapshots) {
        begin_cs =
            bt_message_discarded_packets_borrow_beginning_default_clock_snapshot_const(in_message);

        end_cs = bt_message_discarded_packets_borrow_end_default_clock_snapshot_const(in_message);
        ...
        out_message = bt_message_discarded_packets_create(debug_it->input_iterator, out_stream);
    }
    if (!out_message) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp,
    ...
    prop_avail = bt_message_discarded_packets_get_count(in_message, &discarded_packets);
    if (prop_avail == BT_PROPERTY_AVAILABILITY_NOT_AVAILABLE) {
        bt_message_discarded_packets_set_count(out_message, discarded_packets);
    }
...
static struct bt_param_validation_map_value_entry_descr debug_info_params[] = {
    {"debug-info-field-name", BT_PARAM_VALIDATION_MAP_VALUE_ENTRY_OPTIONAL,
     bt_param_validation_value_descr::makeString()},
    {"debug-info-dir", BT_PARAM_VALIDATION_MAP_VALUE_ENTRY_OPTIONAL,
     bt_param_validation_value_descr::makeString()},
    {"target-prefix", BT_PARAM_VALIDATION_MAP_VALUE_ENTRY_OPTIONAL,
     bt_param_validation_value_descr::makeString()},
    {"full-path", BT_PARAM_VALIDATION_MAP_VALUE_ENTRY_OPTIONAL,
     bt_param_validation_value_descr::makeBool()},
    BT_PARAM_VALIDATION_MAP_VALUE_ENTRY_END};

static bt_component_class_initialize_method_status
init_from_params(struct debug_info_component *debug_info_component, const bt_value *params)
{
    ...
    if (validation_status == BT_PARAM_VALIDATION_STATUS_MEMORY_ERROR) {
        ...
        BT_COMP_LOGE_APPEND_CAUSE(debug_info_component->self_comp, "%s", validate_error);
        goto end;
    }

    BT_ASSERT(params);

    value = bt_value_map_borrow_entry_value_const(params, "debug-info-field-name");
    if (value) {
        debug_info_component->arg_debug_info_field_name = g_strdup(bt_value_string_get(value));
    } else {
        debug_info_component->arg_debug_info_field_name = g_strdup(DEFAULT_DEBUG_INFO_FIELD_NAME);
    }
...
debug_info_comp_init(bt_self_component_filter *self_comp_flt,
                     ...
                     const bt_value *params, void *init_method_data __attribute__((unused)))
{
    ...
    BT_COMP_LOGI("Initializing debug_info component: "
                 "comp-addr=%p, params-addr=%p",
                 self_comp, params);

    debug_info_comp = g_new0(struct debug_info_component, 1);
    if (!debug_info_comp) {
        BT_COMP_LOGE_APPEND_CAUSE(self_comp, "Failed to allocate one debug_info component.");
        goto error;
    }
...
debug_info_supported_mip_versions(bt_self_component_class_filter *self __attribute__((unused)),
...
{
    return static_cast<bt_component_class_get_supported_mip_versions_method_status>(
...
{
    struct debug_info_component *debug_info = static_cast<debug_info_component *>(
    ...
    BT_COMP_LOGI("Finalizing debug_info self_component: comp-addr=%p", self_comp);
                         ...
                         uint64_t capacity, uint64_t *count)
{
    ...
    upstream_iterator_ret_status = bt_message_iterator_next(upstream_iterator, &input_msgs, count);
    if (upstream_iterator_ret_status != BT_MESSAGE_ITERATOR_NEXT_STATUS_OK) {
        /*
         ...
         */
        status =
            static_cast<bt_message_iterator_class_next_method_status>(upstream_iterator_ret_status);
        goto end;
    }
    ...
    for (curr_msg_idx = 0; curr_msg_idx < *count; curr_msg_idx++) {
        out_message = handle_message(debug_info_msg_iter, input_msgs[curr_msg_idx]);
        if (!out_message) {
...
static void debug_info_msg_iter_destroy(struct debug_info_msg_iter *debug_info_msg_iter)
{
    ...
    if (debug_info_msg_iter->msg_iter) {
        bt_message_iterator_put_ref(debug_info_msg_iter->msg_iter);
    }
                         ...
                         bt_self_component_port_output *self_port __attribute__((unused)))
{
    ...
    input_port = bt_self_component_filter_borrow_input_port_by_name(
        debug_info_msg_iter->debug_info_component->self_comp_filter, "in");
    if (!input_port) {
    ...
    msg_iter_status = bt_message_iterator_create_from_message_iterator(self_msg_iter, input_port,
                                                                       &upstream_iterator);
    ...
    debug_info_field_name = debug_info_msg_iter->debug_info_component->arg_debug_info_field_name;

    debug_info_msg_iter->ir_maps =
        trace_ir_maps_create(self_comp, debug_info_field_name, log_level);
    if (!debug_info_msg_iter->ir_maps) {
        status = BT_MESSAGE_ITERATOR_CLASS_INITIALIZE_METHOD_STATUS_MEMORY_ERROR;
        goto error;
    }
    ...
    bt_self_message_iterator_configuration_set_can_seek_forward(
        config, bt_message_iterator_can_seek_forward(debug_info_msg_iter->msg_iter));
...
debug_info_msg_iter_can_seek_beginning(bt_self_message_iterator *self_msg_iter, bt_bool *can_seek)
{
    struct debug_info_msg_iter *debug_info_msg_iter =
        static_cast<struct debug_info_msg_iter *>(bt_self_message_iterator_get_data(self_msg_iter));
    BT_ASSERT(debug_info_msg_iter);

    return static_cast<bt_message_iterator_class_can_seek_beginning_method_status>(
        bt_message_iterator_can_seek_beginning(debug_info_msg_iter->msg_iter, can_seek));
}
...
debug_info_msg_iter_seek_beginning(bt_self_message_iterator *self_msg_iter)
{
    ...
    seek_beg_status = bt_message_iterator_seek_beginning(debug_info_msg_iter->msg_iter);
    if (seek_beg_status != BT_MESSAGE_ITERATOR_SEEK_BEGINNING_STATUS_OK) {
        status =
            static_cast<bt_message_iterator_class_seek_beginning_method_status>(seek_beg_status);
        goto end;
    }
...
void debug_info_msg_iter_finalize(bt_self_message_iterator *it)
{
    ...
    debug_info_msg_iter =
        static_cast<struct debug_info_msg_iter *>(bt_self_message_iterator_get_data(it));
    BT_ASSERT(debug_info_msg_iter);
